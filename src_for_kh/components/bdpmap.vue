...
<script>
    export default {
        name: 'bdp-map',
        props: ['flyData', 'carData', 'coord', 'mapStyle', 'name'],
        data() {
            return {
            };
        },
        computed: {
            geoCoordMap() {
                ...
                const coord = this.coord ? this.coord : {};
                return Object.create(base, coord);
            },
        },
        mounted() {
            const { bdpMap } = this.$refs;
            if (!bdpMap) return;
            this.myChart = this.$echarts.init(bdpMap);
            window.onresize = () => (() => {
                this.myChart.resize();
            })();
            // 绘制飞机图形
            ...
            if (this.carData) {
                this.myChart.setOption(this.drawCar());
            } else {
                this.myChart.setOption(this.draw());
            }
        },
        methods: {
            inits() {
                ...
                const option = {
                    ...
                    geo: {
                        ...
                        itemStyle: {
                            normal: {
                                borderColor: $vm.mapStyle && $vm.mapStyle.borderColor ? $vm.mapStyle.borderColor : '#404a59', // 可设置
                                borderWidth: 1.5,
                                areaColor: $vm.mapStyle && $vm.mapStyle.areaColor ? $vm.mapStyle.areaColor : '#323c48',
                            },
                            emphasis: {
                                areaColor: $vm.mapStyle && $vm.mapStyle.areaColor ? $vm.mapStyle.areaColor : '#323c48', // 可设置
                                // borderWidth: 5,
                                // areaColor: '#32cd32',
                                label: {
            ...
            draw() {
                ...
                // const colorsArr2 = ['#f5dc04', '#FF6347', '#ff6600', '#00CBD8', '#3D8BEB', '#3435EF', '#DA70D6', '#32CD32', '#7130aa', '#2c69c8', '#cfa152', '#50d2e3', '#FFB6C1', '#FFD700'];
                const colorsArr = ['#f5dc04', '#FF99FF', '#FF9900', '#FF3300', '#FF0066', '#CCFFFF', '#CC9966', '#CCFF00', '#CC6600', '#99CCCC', '#9900FF', '#6699FF', '#6699CC', '#33FFFF', '#339999', '#660066', '#FF6347', '#ff6600', '#00CBD8', '#3D8BEB', '#3435EF', '#DA70D6', '#32CD32', '#7130aa', '#2c69c8', '#cfa152', '#50d2e3', '#FFB6C1', '#FFD700'];
                const colors = {};
                let idx = 0;
                if ($vm.flyData) {
                    $vm.flyData.forEach((v) => {
                        /* eslint-disable */
                        //  console.log($vm.geoCoordMap[v.fromName]);
                        let fromName = v.fromName;
                        let toName = v.toName;
                        if (fromName.indexOf('/') !== -1) {
                            const arr1 = fromName.split('/');
                            fromName = arr1[arr1.length - 1];
                        }
                        if (toName.indexOf('/') !== -1) {
                            const arr2 = toName.split('/');
                            toName = arr2[arr2.length - 1];
                        }
                        if ($vm.geoCoordMap[fromName] && $vm.geoCoordMap[toName]) {
                            const geoFrom = $vm.geoCoordMap[fromName];
                            const geoTo = $vm.geoCoordMap[toName];
                            flyGeo[fromName] = v.value;
                            flyGeo[toName] = v.value;
                            const obj = Object.create(v);
                            obj.coords = [geoFrom, geoTo];
                            flyLine.push(obj);
                            if (!colors[fromName]) {
                                colors[fromName] = colorsArr[idx];
                                idx += 1;
                            }
                        } else {
                            console.warn(`无效“流向城市”： ${toName}`);
                        }
                    });
                }
                const series = [];
                const dts = [];
                Object.keys(flyGeo).map((v, i) => {
                    const obj = {};
                    obj.name = v;
                    obj.value = $vm.geoCoordMap[v];
                    dts.push(obj);
                });
                flyLine.forEach((v, i) => {
                    series.push({
                        ...
                        lineStyle: {
                            normal: {
                                color: colors[v.fromName],
                                width: $vm.lineStyle && $vm.lineStyle.width ? $vm.lineStyle.width : 1,
                                opacity: $vm.lineStyle && $vm.lineStyle.opacity ? $vm.lineStyle.opacity : '0.5',
                                curveness: 0.2,
                            },
                        },
                        data: [flyLine[i]],
            ...
            drawCar() {
                ...
                this.carData.forEach((v) => {
                    if ($vm.geoCoordMap[v.name]) {
                        data.push({
                            name: v.name,
                            value: $vm.geoCoordMap[v.name].concat([v.count]),
                        });
                    }
                });
                const series = [
                    {
                        name: $vm.name ? $vm.name : '货车',
                        type: 'effectScatter',
                        coordinateSystem: 'geo',
                        data,
                        symbolSize: 10,
